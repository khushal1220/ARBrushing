...
public class MPTrackingManager : MonoBehaviour
{
    ...
    {
        if (FaceLandmarkParent == null) return;
    ...
    public Transform GetHandLandmark(int key)
    {
        ...
        {
            if (handLandmarkParent == null) return null;
            if (handLandmarkParent.childCount - 1 < key) return null;
    ...
    void handleFace()
    {
        ...
        else
        {
            ...
            scaleFactor = faceScaleFactor * scaleFactor;
            scaleFactor = Mathf.Clamp(scaleFactor, .5f, 1.5f);
            //face.localScale = Vector3.one * scaleFactor;
            face.localScale = Vector3.Lerp(face.localScale, Vector3.one * scaleFactor, smoothingFactor * Time.deltaTime);
